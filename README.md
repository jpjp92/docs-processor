...
- 원본 PDF 다운로드
- 분석 결과 Word 리포트 다운로드 — 요약, 선택 영역 원문, 질의응답을 문서형 서식으로 변환
...
- 표 셀의 `<br>` 등 인라인 HTML을 Word 줄바꿈과 서식으로 정규화
...
기본 모델은 OpenAI `gpt-5.4-mini`, Gemini `gemini-3.5-flash`입니다.

API 키는 서버 `.env`에만 설정합니다. 설정 창에서는 키를 입력하지 않고, 서버 키 연결 여부에 따라 사용 가능한 프로바이더와 모델만 선택합니다.

현재 UI에서 선택하는 모델은 비용과 품질 균형을 기준으로 좁혀 두었습니다.
...
```json
{ "text": "문서 전체 본문", "provider": "gemini", "model": "gemini-3.5-flash" }
```
...
- 요약 출력 한도는 `8,192` 토큰으로 설정해 긴 PDF에서도 Gemini 스트리밍 응답이 중간에 잘리지 않도록 완화합니다.
- SSE 스트림은 마지막 `data:` 조각이 개행 없이 끝나도 누락하지 않도록 처리합니다.
...
- `Word 리포트`: 문서 요약, 선택 영역 추출 텍스트, 질문/답변, 종료 사유 메타데이터 포함
- `HTML 리포트`: 브라우저에서 바로 열어 보기 좋은 단일 HTML 파일. 문서 요약이 최상단에 표시됩니다.

Word 리포트는 모델 답변에 포함된 Markdown을 그대로 텍스트로 붙이지 않고, 제목/인용/목록/표/강조 서식으로 변환합니다.
...
Gemini가 원문 재현 위험으로 `RECITATION` 차단을 반환하는 경우, Refine은 원문 복제 대신 관찰·요약 중심의 안전 프롬프트로 한 번 재시도합니다.
